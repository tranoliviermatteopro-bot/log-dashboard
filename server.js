...
app.post('/api/ingest', requireApiKey, (req, res) => {
  ...
  runQuery(
    'INSERT INTO logs (site_name, level, message, meta, source) VALUES (?, ?, ?, ?, ?)',
...
app.delete('/api/logs', requireAdmin, (req, res) => {
  ...
  if (site) {
    runQuery('DELETE FROM logs WHERE site_name = ?', [site]);
  } else {
...
app.patch('/api/sites/:id', requireAdmin, (req, res) => {
  ...
  runQuery('UPDATE api_keys SET active = ? WHERE id = ?', [active ? 1 : 0, req.params.id]);
  res.json({ ok: true });
});
